...
install-smoke:
	...
	printf '%s\n' '{"id":"caps","method":"akbp.capabilities"}' '{"id":"bad","method":"akbp.search","params":{"query":"release","limit":0}}' | PYTHONPATH=$$TMP/pkg python3 -m akbp_tool_server | python3 -c "import json,sys; rows=[json.loads(line) for line in sys.stdin if line.strip()]; caps=rows[0]['result']; assert caps['features']['method_param_schemas']; assert caps['knowledge_capability']['kind'] == 'agent_knowledge_base'; assert caps['knowledge_capability']['trust_model'] == 'cited_review_gated_memory'; assert 'akbp.import_apply' in caps['methods']; assert rows[1]['error']['code'] == 'invalid_params'; assert 'limit must be between 1 and 100' in rows[1]['error']['details']['type_errors']; print('tool server module install ok')"; \
	printf '%s\n' '{"id":"caps","method":"akbp.capabilities"}' | PATH=$$TMP/pkg/bin:$$PATH PYTHONPATH=$$TMP/pkg akbp-tool-server | python3 -c "import json,sys; row=json.loads(sys.stdin.readline()); caps=row['result']; assert caps['features']['method_param_schemas']; assert caps['knowledge_capability']['kind'] == 'agent_knowledge_base'; assert caps['knowledge_capability']['retrieval']['startup_method'] == 'akbp.session.start'; assert 'akbp.import_apply' in caps['methods']; print('tool server console install ok')"
