...
- `invalid-param-rejection`: validate schema-backed `invalid_params` details for unknown, missing, wrong-typed, wrong item-typed, out-of-range, oversized string, import/export file-param, source verification id, cite claim id, claim relation id, audit limit, read-method limit, lifecycle method params, and unsupported enum method parameters before CLI execution.
