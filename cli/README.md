...
The generated config includes the server command, knowledge-base path, startup `akbp.capabilities` request, required workflow profile, `akbp.doctor` health check, session-start method, structured response contract, quality gates, tool-protocol bridge allowlists, and safety rules. Adapters should disable unavailable flows when `result.negotiation.satisfied` is false, follow `doctor.adapter_readiness.recommended_profile` when the knowledge base is not ready for reviewed writes, show `doctor.next_steps` when `ready_for_adapter` is false, and branch on `ok` plus `error.code` instead of prose.

The `quality_gates` section tells adapters when startup context is safe to use in a plan: require at least one recalled item, require citations, surface warnings, and continue without recalled memory when context is empty or uncited. Reviewed-write configs also list the preview fields an adapter must show before retrying with `approved:true`.
