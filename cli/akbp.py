...
def cmd_discover(args: argparse.Namespace) -> int:
    ...
    tool_protocol_bridge_preflight = {
        "format": "akbp-discovery-tool-protocol-bridge-v1",
        "purpose": "Let installers decide the safe bridge path from akbp discover before generating a full client-config manifest.",
        "research_signal": "Recent agent-memory adoption is pulled by easy tool-protocol server setup; AKBP should answer with a fast read-only bridge proof that keeps citations, envelopes, and review gates intact.",
        "safe_default": "read_only_bridge",
        "next_command": f"akbp --path {kb_arg} client-config --profile read-only",
        "runnable_preflight": "./examples/tool-protocol-bridge/run.sh",
        "read_only_methods": [
            "akbp.capabilities",
            "akbp.doctor",
            "akbp.session.start",
        ...
        "must_preserve": [
            "response envelope ok/result/error",
            ...
            "dry-run review metadata before any approved write",
        ],
        "enable_reviewed_writes_after": [
            "doctor --profile reviewed-writes passes",
            "the bridge can render dry-run preview fields outside the model tool call",
            "approved apply repeats the exact reviewed method, path, and params",
    ...
    adapter_prompt_contract = {
        ...
        "context_use_report": {
            ...
            "purpose": "Make memory-assisted planning auditable before an installer or adapter lets recalled AKBP context influence the host runtime.",
        ...
        "startup_trust_gate": {
            ...
            "fail_closed_on": [
                ...
                "result.quality.fail_on_warnings is not true for startup preflight",
                "result.quality.ok is false because warnings were present",
...
def cmd_client_config(args: argparse.Namespace) -> int:
    ...
    adapter_prompt_contract = {
        ...
        "startup_trust_gate": {
            ...
            "warning_action": "Request fail_on_warnings:true for startup checks; if warnings are present, continue without recalled AKBP memory unless a human explicitly downgrades the gate.",
            "fallback_action": "Continue without recalled AKBP memory, do not invent prior decisions, and keep write-capable tools disabled until startup trust passes.",
    ...
    host_capability_descriptor = {
        ...
        "tool_protocol_memory_capability": {
            "format": "akbp-tool-protocol-memory-capability-v1",
            "purpose": "Map AKBP into hosts that classify memory-like capabilities without reducing it to an opaque memory server.",
            "candidate_labels": [
                "memory",
                "agent_memory",
                "project_knowledge",
                "durable_agent_knowledge",
            ],
            "required_semantics": [
                "local artifacts remain the durable source of truth",
                "startup recall returns citations or the host continues without recalled memory",
                "write-capable flows require dry_run preview and exact approved:true apply",
                "structured errors, warnings, and budget fields are preserved",
                "export and import checks do not depend on host-local memory state",
            ],
            "read_only_profile": "read_only",
            "write_profile": "reviewed_write",
            "do_not_advertise_as": [
                "automatic background memory",
                "uncited chat history store",
                "bridge-owned vector database",
                "host-only memory cache",
            ],
            "host_registry_rule": "Advertise AKBP as a memory capability only when the host can preserve citations, response envelopes, write review metadata, and the selected profile contract.",
            "fallback": "If the host's memory capability type cannot express citations or reviewed writes, register only read-only startup context tools.",
        },
        "transport": "stdio-jsonl",
    ...
    config = {
        ...
        "memory_landscape_fit": {
            ...
            "compaction_survival_claim_gate": {
                ...
                "required_checks": [
                    ...
                    "adapter_prompt_contract.context_use_report records used item ids, citation ids, quality status, budget truncation, warnings, and fallback reason",
                ],
                "must_preserve": [
                    "result.context.items[].id",
                    "result.context.items[].citations",
        ...
        "context_compaction_recovery": {
            ...
            "recovery_sequence": [
                ...
                },
                {
                    ...
                    "required": True,
                    "expect": "the host records used_akbp_context, item ids, citation ids, quality status, budget truncation, surfaced warnings, and fallback reason",
                },
                {
        ...
        "adapter_contract_harness": {
            ...
            "proves": [
                ...
                "startup budget warnings can fail closed before recalled context is trusted",
                "context-use reports preserve memory-use audit fields before planning",
