...
4. Confirm startup context is ready and returns citations.
5. Generate a portable read-only client config that uses `<AKBP_KB_PATH>`
   instead of committing a local absolute path.
6. Prove `akbp.remember` rejects unapproved durable writes.
