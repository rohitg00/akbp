...
The example creates a small markdown note folder, registers each file as source evidence, previews the durable claims, proves unapproved writes are blocked, applies only reviewed claims, verifies source freshness, rebuilds the index, and retrieves cited startup context.
...
6. Run `akbp.source.verify` with `fail_on_issue:true` before trusting the promoted notes.
7. Retrieve context with citations before letting a later agent plan from it.
