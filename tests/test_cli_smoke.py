...
class AkbpCliSmokeTest(unittest.TestCase):
    ...
    def test_client_config_generates_negotiated_stdio_profile(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn("persistent memory makes unreliable coding-agent sessions safer", claim_audit)
            ...
            self.assertIn("compaction-handoff-recall", claim_audit["memory survives compaction or session restart"]["evidence"])
            self.assertIn("reliability gate before planning", claim_audit["persistent memory makes unreliable coding-agent sessions safer"]["akbp_check"])
            self.assertIn("inherited-repo-intake", claim_audit["persistent memory makes unreliable coding-agent sessions safer"]["evidence"])
            ...
            git_review = landscape["git_reviewable_promotion_flow"]
            self.assertEqual(git_review["format"], "akbp-git-reviewable-promotion-flow-v1")
            self.assertIn("Git-backed memory comparisons", git_review["purpose"])
            self.assertIn("pull-request review", git_review["use_when"][1])
            review_steps = {step["step"]: step for step in git_review["promotion_sequence"]}
            self.assertIn("select_scope", review_steps)
            self.assertIn("register_evidence", review_steps)
            self.assertIn("preview_write", review_steps)
            self.assertIn("review_diff", review_steps)
            self.assertIn("approved_apply", review_steps)
            self.assertIn("verify_bundle", review_steps)
            self.assertIn("dry_run:true", review_steps["preview_write"]["check"])
            self.assertIn("approved:true", review_steps["approved_apply"]["check"])
            self.assertIn("export-check", review_steps["verify_bundle"]["check"])
            self.assertIn("raw transcripts", git_review["must_not_commit"][0])
            self.assertIn("source id or citation", git_review["fail_closed_when"][1])
            self.assertIn("import-check", git_review["fallback"])
            temporal_graph = landscape["temporal_graph_claim_gate"]
            self.assertEqual(temporal_graph["format"], "akbp-temporal-graph-claim-gate-v1")
            self.assertIn("knowledge-graph memory claims", temporal_graph["purpose"])
            self.assertIn("temporal knowledge graph memory", temporal_graph["run_before_claiming"])
            self.assertIn("claims/claims.jsonl with cited source ids", temporal_graph["required_artifacts"])
            self.assertIn("graph/relations.jsonl", temporal_graph["required_artifacts"][2])
            self.assertIn("akbp conformance --level 3", temporal_graph["required_checks"])
            self.assertIn("graph-jsonl-records", temporal_graph["required_checks"][2])
            self.assertIn("claim lifecycle status", temporal_graph["must_preserve"][1])
            self.assertIn("opaque memory-server database", temporal_graph["fail_closed_when"][3])
            self.assertIn("ephemeral graph index", temporal_graph["fallback"])
            compaction_gate = landscape["compaction_survival_claim_gate"]
            self.assertEqual(compaction_gate["format"], "akbp-compaction-survival-claim-gate-v1")
            self.assertIn("compaction-survival", compaction_gate["purpose"])
            self.assertIn("memory survives compaction", compaction_gate["run_before_claiming"])
            self.assertEqual(compaction_gate["required_request"]["method"], "akbp.session.start")
            self.assertTrue(compaction_gate["required_request"]["params"]["require_citations"])
            self.assertTrue(compaction_gate["required_request"]["params"]["fail_on_warnings"])
            self.assertIn("compaction-handoff-recall", compaction_gate["required_checks"][0])
            self.assertIn("adapter_prompt_contract.context_use_report", compaction_gate["required_checks"][2])
            self.assertIn("result.context.items[].citations", compaction_gate["must_preserve"])
            self.assertIn("uncited compacted chat summary", compaction_gate["fail_closed_when"][3])
            self.assertIn("fresh session", compaction_gate["fallback"])
            ...
            freshness_probe = config["context_freshness_probe"]
            self.assertEqual(freshness_probe["format"], "akbp-context-freshness-probe-v1")
            self.assertEqual(freshness_probe["safe_default"], "verify_sources_before_planning_from_recalled_context")
            self.assertIn("planning_from_recalled_context", freshness_probe["required_before"])
            self.assertEqual(freshness_probe["probe_sequence"][0]["request"]["method"], "akbp.source.verify")
            self.assertTrue(freshness_probe["probe_sequence"][0]["request"]["params"]["fail_on_issue"])
            ...
            reliability_gate = read_only["memory_landscape_fit"]["coding_agent_reliability_gate"]
            self.assertEqual(reliability_gate["format"], "akbp-coding-agent-reliability-gate-v1")
            self.assertIn("inherited repository", reliability_gate["run_when"][0])
            reliability_steps = {step["step"]: step for step in reliability_gate["required_sequence"]}
            self.assertIn("discover_scope", reliability_steps)
            self.assertIn("readiness", reliability_steps)
            self.assertIn("cited_startup", reliability_steps)
            self.assertIn("context_use_report", reliability_steps)
            self.assertIn("reviewed_promotion", reliability_steps)
            self.assertIn("require_citations:true", reliability_steps["cited_startup"]["check"])
            self.assertIn("adapter_prompt_contract.context_use_report", reliability_steps["context_use_report"]["check"])
            self.assertIn("result.context.quality.trusted_for_planning", reliability_gate["must_preserve"])
            self.assertIn("budget-truncated", reliability_gate["fail_closed_when"][0])
            self.assertIn("source-truth-drift", " ".join(reliability_gate["verification"]))
            self.assertIn("Continue without recalled AKBP memory", reliability_gate["fallback"])
    ...
    def test_discover_finds_nearest_kb_and_reports_trust_boundary(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(inherited_intake["example"], "./examples/inherited-repo-intake/run.sh")
            freshness_probe = discovered["context_freshness_probe"]
            self.assertEqual(freshness_probe["format"], "akbp-context-freshness-probe-v1")
            self.assertEqual(freshness_probe["probe_sequence"][0]["request"]["method"], "akbp.source.verify")
            self.assertEqual(freshness_probe["probe_sequence"][1]["request"]["method"], "akbp.session.start")
    ...
    def test_ingest_dry_run_previews_redacted_writes_without_creating_kb(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(data["dry_run"])
            self.assertTrue(data["redacted"])
            self.assertEqual(data["signal_refs"][0]["text"], "Import Preview")
            self.assertEqual(data["signal_refs"][0]["line"], 1)
            self.assertEqual(data["signal_refs"][0]["kind"], "heading")
            self.assertTrue(any(ref["text"] == "Decision: preview before write." for ref in data["signal_refs"]))
