...
class ToolServerTest(unittest.TestCase):
    ...
    def test_response_schema_documents_write_review_shapes(self):
        ...
        self.assertIn("trust_boundary", defs["status_result"]["required"])
    ...
    def test_status_context_and_capabilities_methods(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(lines[0]["result"]["features"]["session_boundary_contract"])
            ...
            contract = boundary["promotion_contract"]
            self.assertIn("workflow", contract["durable_candidate_types"])
            self.assertIn("review_required", contract["preview_must_surface"])
            self.assertIn("would_write", contract["preview_must_surface"])
            self.assertTrue(contract["apply_requires_same_request"])
            self.assertTrue(contract["apply_requires_reviewed_summary"])
            self.assertIn("uncited_claims", contract["reject_inputs"])
            ...
            self.assertEqual(lines[1]["result"]["adapter_readiness"]["recommended_profile"], "startup_context")
            self.assertTrue(lines[1]["result"]["adapter_readiness"]["startup_context_ready"])
            self.assertFalse(lines[1]["result"]["adapter_readiness"]["read_only_ready"])
